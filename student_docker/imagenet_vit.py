...
used by the competition pipeline. The 100-way task uses the immutable
`splits/student_split.pt` manifest through `utils/data.py`: `released` is the
only training split, public `validation` is scoring-only, and private
`test` is absent from the student server.
"""

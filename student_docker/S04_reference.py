...
def main(config=None, after_step=None, gradient_step=None):
    if config is None:
    ...
    for step in range(steps):
        ...
        with torch.autocast(device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
            ...
            retain_loss = w["feat_r"] * l_feat_r + w["kd_r"] * l_kd_r + w["ce_r"] * l_ce_r
            forget_loss = w["remap_f"] * l_remap_f + w["ce_f"] * l_ce_f + w["cka_f"] * l_cka_f
        ...
        if gradient_step is None:
            ...
            gradient_step(retain_loss, forget_loss, params, step + 1)
        if t.get("clip_grad"):

# S11 — state-triggered cosine tail on noce control
...
class StateTriggeredCosineTail:
    """Start cosine LR decay once the forget objective is stably saturated."""

    def __init__(self, *, total_steps, base_lr, final_lr, floor, min_step,
                 window, off_ratio_threshold, ema_beta):
        if total_steps < 2 or not 0 <= min_step < total_steps - 1:
            raise ValueError("min_step must leave at least one tail update")
        if window < 1:
            raise ValueError("window must be positive")
        if not 0.0 <= final_lr <= base_lr:
            raise ValueError("final_lr must be between zero and base_lr")
        if not 0.0 <= off_ratio_threshold <= 1.0:
            raise ValueError("off_ratio_threshold must be within [0, 1]")
        ...
        self.off_history = deque(maxlen=int(window))
        self.window = int(window)
        self.off_ratio_threshold = float(off_ratio_threshold)
    ...
    @property
    def off_ratio(self):
        return sum(self.off_history) / len(self.off_history) if self.off_history else 0.0

    def update(self, step, *, raw_cka_f):
        if not 0 <= step < self.total_steps:
            raise ValueError("step must be within the optimizer-update range")
        value = float(raw_cka_f)
        self.cka_ema = (value if self.cka_ema is None else
                        self.ema_beta * self.cka_ema + (1 - self.ema_beta) * value)
        self.off_history.append(value <= self.floor)
        triggered_now = False
        ready = (step >= self.min_step and step < self.total_steps - 1
                 and len(self.off_history) == self.window)
        if (self.trigger_step is None and ready and
                (self.cka_ema <= self.floor or self.off_ratio >= self.off_ratio_threshold)):
            self.trigger_step = step
            triggered_now = True
        if self.trigger_step is None:
            return self.base_lr, triggered_now
        progress = (step - self.trigger_step) / (self.total_steps - 1 - self.trigger_step)
        cosine = 0.5 * (1.0 + math.cos(math.pi * progress))
        lr = self.final_lr + (self.base_lr - self.final_lr) * cosine
        return float(lr), triggered_now
...
def main():
    ...
    state_tail = None
    if str(t.get("lr_schedule", "constant")).lower() == "state_triggered_cosine_tail":
        trigger_floor = float(t.get("cka_floor", 0) or 0)
        if trigger_floor <= 0:
            raise ValueError("state-triggered tail requires a positive cka_floor")
        state_tail = StateTriggeredCosineTail(
            total_steps=steps,
            base_lr=float(t["lr"]),
            final_lr=float(t.get("lr_final", 0.0)),
            floor=trigger_floor,
            min_step=int(t.get("lr_trigger_min_step", 3600)),
            window=int(t.get("lr_trigger_window", 200)),
            off_ratio_threshold=float(t.get("lr_trigger_off_ratio", 0.40)),
            ema_beta=float(t.get("lr_trigger_ema_beta", 0.98)),
        )
        print("S11 state tail armed: "
              f"min={state_tail.min_step}, window={state_tail.window}, "
              f"off_ratio={state_tail.off_ratio_threshold:.2f}, "
              f"ema_beta={state_tail.ema_beta:.2f}, final_lr={state_tail.final_lr:.3g}")
    ...
    for step in range(steps):
        ...
        if state_tail is not None:
            lr_now, triggered_now = state_tail.update(
                step, raw_cka_f=float(raw_cka_f.detach()))
            for group in opt.param_groups:
                group["lr"] = lr_now
            if triggered_now:
                print(f"S11 tail triggered at step {step}: "
                      f"cka_ema={state_tail.cka_ema:.4f}, "
                      f"off_ratio={state_tail.off_ratio:.3f}", flush=True)
